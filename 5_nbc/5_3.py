...
def main():
	...
	for f in F:
		print ('f = {}'.format(f))
		train_acc, test_acc = nbc(df_training_org, df_testing, f, debug=False, print_=True)
	...
	plot_name = 'plot_5_3.png'

	fig, ax = plt.subplots()
	line1, = ax.plot(F, training_accuracies, label='train')
	line1.set_dashes([2, 2, 10, 2]) 
	line2, = ax.plot(F, test_accuracies, label='test')
	ax.legend()
	title='Training and Test Accuracies for different Sampling Fraction f'
	plt.xlabel('f')
	plt.ylabel('Accuracy')
	plt.title(title)
